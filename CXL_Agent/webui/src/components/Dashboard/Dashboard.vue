    ...
    <v-dialog v-model="dialogNewDiscoveredDevices" max-width="600px">
      <v-card>
        <v-alert
          ...
          variant="tonal"
          text="Here are the discovered devices. You can add them to CXL Agent by selecting them and clicking the 'Add' button."
        ></v-alert>
        <v-card-text class="scrollable-content">
          New Discovered Switches:
          <v-checkbox
...
export default {
  ...
  setup() {
    ...
    const currentTitle = computed(() =>
      currentGraph.value === "controlPlane"
        ? "CXL Agent Ethernet Connections"
        : "CXL Agent CXL Connections"
    );
