...
export const useBladeResourceStore = defineStore('bladeResource', {
  state: () => ({
    ...
    // Optional UI filter (set by clicking ResourceId in Memory table)
    resourceIdFilter: null as string | null,
  }),

  actions: {
    setResourceIdFilter(resourceId: string | null) {
      this.resourceIdFilter = resourceId;
    },
