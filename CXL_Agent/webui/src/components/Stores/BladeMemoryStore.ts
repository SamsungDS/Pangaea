...
import { MemoryRegion, ComposeMemoryRequest, ComposeMemoryByResourceRequest, AssignMemoryRequest } from "@/axios/api";
...
export const useBladeMemoryStore = defineStore('bladeMemory', {
  state: () => ({
    ...
    composeMemoryByResourceError: null as unknown,
  }),
  ...
  actions: {
    ...
    async composeMemoryByResource(applianceId: string, bladeId: string, request: ComposeMemoryByResourceRequest) {
      const defaultApi = getDefaultApi();
      // Reset the error before each compose-by-resource operation
      this.composeMemoryByResourceError = null;
      try {
        const response = await defaultApi!.bladesComposeMemoryByResource(
          applianceId,
          bladeId,
          request
        );
        ...
        if (axios.isAxiosError(error)) {
          this.composeMemoryByResourceError = error.message;
          if (error.response) {
            this.composeMemoryByResourceError = error.response?.data.status.message + " (" + error.response?.request.status + ")";
          }
        } else {
          this.composeMemoryByResourceError = error;
        }
        console.error("Error compose memory by resource:", error);
      }
    },
