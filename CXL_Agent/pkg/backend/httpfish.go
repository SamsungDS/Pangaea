...
func (session *Session) queryWithJSON(operation HTTPOperationType, path string, jsonData map[string]interface{}) Response {
	...
	if session.client == nil {
		...
		session.client = &http.Client{Transport: tr, Timeout: 60 * time.Second} //device power off will present as timeout
		//Time out Changed : 10 --> 20 --> 60 (due to vppb bind & switch reset)
	}
...
func findResourceState(compositionState *string, reserved bool) *ResourceState {
	...
	} else if *compositionState == COMPOSITION_STATE_SHARED {
		resourceState = ResourceShared
	}
...
func (service *httpfishService) GetResourceIdByVppbId(ctx context.Context, settings *ConfigurationSettings, req *GetResourceIdByVppbIdRequest) (string, error) {
