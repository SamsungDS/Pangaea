...
func (b *Blade) AssignMemory(ctx context.Context, r *RequestAssignMemory) (*openapi.MemoryRegion, error) {
	...
	if len(memory.details.ResourceIds) != 0 {
		resourcesToUpdate = memory.details.ResourceIds
	} else {
...
func (b *Blade) ComposeMemoryByResource(ctx context.Context, portId string, resourceIds []string, vppbId string) (*openapi.MemoryRegion, error) {
	logger := klog.FromContext(ctx)
	...
	if errAlloc != nil {
		newErr := fmt.Errorf("allocate memory (backend) failure during compose by resource: appliance [%s] blade [%s] resourceIds [%s]: %w", b.ApplianceId, b.Id, resourceIds, errAlloc)
	...
	memory.details.ResourceIds = append(memory.details.ResourceIds, resourceIds...)
...
func (b *Blade) FreeMemoryById(ctx context.Context, memoryId string) (*openapi.MemoryRegion, error) {
	...
	if !exists {
		newErr := fmt.Errorf("appliance [%s] blade [%s] don't contain memory with id [%s]", b.ApplianceId, b.Id, memoryId)
	...
	memoryDetails, err := memory.GetDetails(ctx)
	if err != nil {
		newErr := fmt.Errorf("free memory by id (backend) failure on appliance [%s] blade [%s] memory [%s]: %w", b.ApplianceId, b.Id, memoryId, err)
		return nil, &common.RequestError{
			StatusCode: common.StatusBladeFreeMemoryFailure,
			Err:        newErr,
		}
	}
	resourceIds := memoryDetails.ResourceIds
	if len(resourceIds) != 0 {
		resourcesToUpdate = resourceIds
	} else {
		resourcesToUpdate = b.GetAllResourceIds(ctx)
	}
	for _, resourceId := range resourcesToUpdate {
...
func (b *Blade) GetPortById(ctx context.Context, portId string) (*CxlBladePort, error) {
	...
	// If only "<portNumber>" case (not "port<portNumber>")
	if !strings.Contains(portId, "port") {
		portId = "port" + portId
	}
	if !b.IsOnline(ctx) {
		// If blade not online, not an error.  Just no information to return.
...
// GetResourceIdByVppbId - Returns the resource ID associated with the given VPPB ID
func (b *Blade) GetResourceIdByVppbId(ctx context.Context, vppbId string) (string, error) {
	logger := klog.FromContext(ctx)
	logger.V(4).Info(">>>>>> GetResourceByVppbId: ", "vppbId", vppbId, "bladeId", b.Id, "applianceId", b.ApplianceId)

	if !b.IsOnline(ctx) {
		logger.V(2).Info("blade is offline, no resource to return", "vppbId", vppbId, "bladeId", b.Id, "applianceId", b.ApplianceId)
		return "", nil
	}

	// Call backend layer
	resourceId, err := b.GetResourceIdByVppbIdBackend(ctx, vppbId)
	if err != nil {
		logger.Error(err, "failure: get resource id by vppbId (backend call failed)", "vppbId", vppbId, "bladeId", b.Id, "applianceId", b.ApplianceId)
		return "", err
	}

	logger.V(2).Info("success: get resource id by vppbId", "vppbId", vppbId, "resourceId", resourceId, "bladeId", b.Id, "applianceId", b.ApplianceId)

	return resourceId, nil
}

// GetResourceIdByVppbIdBackend - Calls backend to get resource by VPPB ID
func (b *Blade) GetResourceIdByVppbIdBackend(ctx context.Context, vppbId string) (string, error) {
	logger := klog.FromContext(ctx)
	logger.V(4).Info(">>>>>> GetResourceByVppbIdBackend: ", "vppbId", vppbId, "bladeId", b.Id, "applianceId", b.ApplianceId)

	req := &backend.GetResourceIdByVppbIdRequest{
		VppbId: vppbId,
	}
	responseId, err := b.backendOps.GetResourceIdByVppbId(ctx, &backend.ConfigurationSettings{}, req)
	if err != nil {
		newErr := fmt.Errorf("get resource id by vppbId [%s] (backend) failure on blade [%s]: %w", vppbId, b.Id, err)
		logger.Error(newErr, "failure: get resource by vppbId (backend)")
		return "", &common.RequestError{StatusCode: common.StatusBackendInterfaceFailure, Err: newErr}
	}
