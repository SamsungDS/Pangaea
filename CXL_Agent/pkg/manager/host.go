...
func (h *Host) addPortById(ctx context.Context, backendPortId string) (*CxlHostPort, error) {
	...
	// Add the new object
	// only upstream port whose serialNumber is identical to hostToSwitch json's serialNumber is added to HostPort.port
	if port.CxlSn == HostToSwitchMap[h.Id].CxlSerialNumber {
...
func (h *Host) initMemoryDevices(ctx context.Context) error {
	...
	for physicalDeviceId, logicalDeviceIds := range deviceIdMap {
		// Only port which is not added to port of cfm(might be dsp port) will be added to memoryDevice
		if _, exist := h.Ports["port"+physicalDeviceId]; exist {
			continue
		}
...
var HostToSwitchMap = map[string]HostData{}
