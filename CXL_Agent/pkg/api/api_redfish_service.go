...
func (cfm *CfmApiService) RedfishV1ChassisChassisIdGet(ctx context.Context, chassisId string) (redfishapi.ImplResponse, error) {
	hostInfo, err := manager.GetHostById(ctx, chassisId)
	if err != nil {
		return formatRedfishErrorResp(ctx, err.(*common.RequestError))
	}

	hostIp := hostInfo.GetNetIp()

	// stores the mapping relationship between host and switch in hostToSwitchDataStore.json.
	// currently, the usp is not found in the host's lspci output, and the serial number is also not parsed correctly.
	// no logic is currently in place to determine which switch's usp is connected to a given host.
	// for now, assumes one usp is mapped per host.
	// TODO: Handle cases where multiple PCI BDFs on a host are mapped to usp devices.
	linkedFabric := manager.HostToSwitchMap[chassisId].FabricName
	linkedSwitch := manager.HostToSwitchMap[chassisId].SwitchName
	linkedUspNumber := manager.HostToSwitchMap[chassisId].LinkedUspNumber

	VCSs := []redfishapi.OdataV4IdRef{}
	VCSs = append(VCSs, redfishapi.OdataV4IdRef{
		OdataId: fmt.Sprintf("/redfish/v1/Fabrics/%s/Switches/%s/VCSs/vcs%s", linkedFabric, linkedSwitch, linkedUspNumber),
	})
	...
	return redfishapi.Response(200, redfishapi.ChassisV1250Chassis{
		...
		Links: redfishapi.ChassisV1250Links{
			Oem: redfishapi.ChassisV1250LinksOem{
				IP:   &hostIp,
				VCSs: VCSs,
			},
		},
...
func (cfm *CfmApiService) RedfishV1FabricsFabricIdConnectionsPost(ctx context.Context, fabricId string, connectionV131Connection redfishapi.ConnectionV131Connection) (redfishapi.ImplResponse, error) {
	...
	if len(appliance.GetAllBladeIds()) == 0 {
		newErr := fmt.Errorf("error: Blade does not exist")
		return redfishapi.Response(http.StatusBadRequest, newErr), newErr
	}
	for bladeId, blade := range appliance.GetBlades(ctx) {
		if len(blade.GetAllMemoryIds(ctx)) == 0 {
			newErr := fmt.Errorf("error: Memory does not exist")
			return redfishapi.Response(http.StatusBadRequest, newErr), newErr
		}
...
func (cfm *CfmApiService) RedfishV1FabricsFabricIdSwitchesSwitchIdGet(ctx context.Context, fabricId string, switchId string) (redfishapi.ImplResponse, error) {
	...
	return redfishapi.Response(200, redfishapi.SwitchV1100Switch{
		OdataContext: "",
		OdataEtag:    "",
		OdataId:      "/redfish/v1/Fabrics/" + fabricId + "/Switches/" + switchId,
		OdataType:    "#Switch.v1_10_0.Switch",
		AssetTag:     nil,
		CXL: redfishapi.SwitchV1100Cxl{
			MaxVCSsSupported:   0,
			NumberOfBoundvPPBs: 0,
			PBRCapable:         false,
			TotalHDMDecoders:   0,
			TotalNumbervPPBs:   0,
			VCS:                redfishapi.SwitchV1100VcsSwitch{},
			VirtualCXLSwitches: redfishapi.SwitchV1100CxlVirtualCxlSwitches{
				OdataId: fmt.Sprintf("/redfish/v1/Fabrics/%s/Switches/%s/VCSs", fabricId, switchId),
			},
		},
		Certificates:            redfishapi.SwitchV1100SwitchCertificates{},
		CurrentBandwidthGbps:    nil,
		Description:             "",
		DomainID:                nil,
		Enabled:                 false,
		EnvironmentMetrics:      redfishapi.SwitchV1100SwitchEnvironmentMetrics{},
		FirmwareVersion:         nil,
		Id:                      switchId,
		IndicatorLED:            nil,
		IsManaged:               nil,
		Links:                   redfishapi.SwitchV1100Links{},
		Location:                redfishapi.ResourceLocation{},
		LocationIndicatorActive: nil,
		LogServices:             redfishapi.SwitchV1100SwitchLogServices{},
		Manufacturer:            nil,
		MaxBandwidthGbps:        nil,
		//Measurements:            nil,
		Metrics:              redfishapi.SwitchV1100SwitchMetrics{},
		Model:                nil,
		Name:                 switchId,
		Oem:                  nil,
		PartNumber:           nil,
		Ports:                redfishapi.SwitchV1100SwitchPorts{OdataId: fmt.Sprintf("/redfish/v1/Fabrics/%s/Switches/%s/Ports", fabricId, switchId)},
		PowerState:           nil,
		Redundancy:           nil,
		RedundancyodataCount: 0,
		SKU:                  nil,
...
func (cfm *CfmApiService) RedfishV1FabricsFabricIdSwitchesSwitchIdPortsGet(ctx context.Context, fabricId string, switchId string) (redfishapi.ImplResponse, error) {
	...
	sort.Slice(portIds, func(i, j int) bool {
		num1, _ := strconv.Atoi(re.FindString(portIds[i]))
		num2, _ := strconv.Atoi(re.FindString(portIds[j]))
		return num1 < num2
	})
	members := []redfishapi.OdataV4IdRef{}
...
func (cfm *CfmApiService) RedfishV1FabricsFabricIdSwitchesSwitchIdPortsPortIdGet(ctx context.Context, fabricId string, switchId string, portId string) (redfishapi.ImplResponse, error) {
	...
	var portType redfishapi.PortV1110PortType
	if details.PortType == "USP" {
		portType = redfishapi.PORTV1110PORTTYPE_UPSTREAM_PORT
	} else if details.PortType == "DSP" {
		portType = redfishapi.PORTV1110PORTTYPE_DOWNSTREAM_PORT
	}
	currentSpeedGbpsFloat32 := float32(details.CurrentSpeedGbps)
...
func (cfm *CfmApiService) RedfishV1SystemsComputerSystemIdMemoryDomainsMemoryDomainIdMemoryChunksMemoryChunksIdGet(ctx context.Context, computerSystemId string, memoryDomainId string, memoryChunksId string) (redfishapi.ImplResponse, error) {
	...
	// if assigned, parse usp info
	var endPoints = []redfishapi.OdataV4IdRef{}
	if details.MemoryAppliancePort != "" {
		endPoints = append(endPoints, redfishapi.OdataV4IdRef{
			OdataId: "/redfish1/v1/Fabrics/" + computerSystemId + "/Endpoints/" + memoryDomainId + "up" + details.MemoryAppliancePort,
		})
	}
	...
	return redfishapi.Response(200, redfishapi.MemoryChunksV161MemoryChunks{
		...
		Links: redfishapi.MemoryChunksV161Links{
			Endpoints:           endPoints,
			EndpointsodataCount: int64(len(endPoints)),
		},
		// MediaLocation:             "",
		MemoryChunkSizeMiB: &memorySize,
		Name:               memoryChunksId,
...
func (cfm *CfmApiService) RedfishV1SystemsComputerSystemIdMemoryDomainsMemoryDomainIdMemoryChunksPost(ctx context.Context, computerSystemId string, memoryDomainId string, memoryChunksV161MemoryChunks redfishapi.MemoryChunksV161MemoryChunks) (redfishapi.ImplResponse, error) {
	...
	for _, dpInfo := range memoryChunksV161MemoryChunks.InterleaveSets {
		if dpInfo.Memory.OdataId == "" {
			newErr := fmt.Errorf("link to memory is required")
			return redfishapi.Response(http.StatusBadRequest, newErr), newErr
		}
		if dpInfo.OffsetMiB == nil {
			newErr := fmt.Errorf("OffsetMiB field is required")
			return redfishapi.Response(http.StatusBadRequest, newErr), newErr
		}
...
func (cfm *CfmApiService) RedfishV1FabricsFabricIdSwitchesSwitchIdVCSsGet(ctx context.Context, fabricId string, switchId string) (redfishapi.ImplResponse, error) {
	appliance, err := manager.GetApplianceById(ctx, fabricId)
	if err != nil {
		return formatRedfishErrorResp(ctx, err.(*common.RequestError))
	}
	...
	for _, portId := range portIds {
		port, _ := blade.GetPortById(ctx, portId)
		detail, _ := port.GetDetails(ctx)
		if detail.PortType != "DSP" {
			members = append(members, redfishapi.OdataV4IdRef{
				OdataId: fmt.Sprintf("/redfish/v1/Fabrics/%s/Switches/%s/VCSs/%s", fabricId, switchId, fmt.Sprintf("vcs%s", strings.Replace(portId, "port", "", 1))),
			})
		}
	}

	return redfishapi.Response(200, redfishapi.VirtualCxlSwitchCollectionVirtualCxlSwitchCollection{
		// OdataContext:         "",
		// OdataEtag:            "",
		OdataId:   "/redfish/v1/Fabrics/" + fabricId + "/Switches/" + switchId + "/VCSs",
		OdataType: "#VCSs.V1_0_0.VCSs",
		// Description:          "",
		Members:           members,
		MembersodataCount: int64(len(members)),
		// MembersodataNextLink: "",
...
func (cfm *CfmApiService) RedfishV1FabricsFabricIdSwitchesSwitchIdVCSsVcsIdGet(ctx context.Context, fabricId string, switchId string, vcsId string) (redfishapi.ImplResponse, error) {
	appliance, err := manager.GetApplianceById(ctx, fabricId)
	if err != nil {
		return formatRedfishErrorResp(ctx, err.(*common.RequestError))
	}

	portId := strings.Replace(vcsId, "vcs", "port", 1)
	blade, err := appliance.GetBladeById(ctx, switchId)
	if err != nil {
		return formatRedfishErrorResp(ctx, err.(*common.RequestError))
	}
	...
	//details, err := port.GetDetails(ctx)
	//if err != nil {return formatRedfishErrorResp(ctx, err.(*common.RequestError))}

	links := redfishapi.VirtualCxlSwitchV100Links{
		Endpoints:           []redfishapi.OdataV4IdRef{{OdataId: fmt.Sprintf("/redfish/v1/Fabrics/%s/Endpoints/%s", fabricId, switchId+strings.Replace(vcsId, "vcs", "up", 1))}},
		EndpointsodataCount: 1,
		Oem:                 nil,
		Port:                "/redfish/v1/Fabrics/" + fabricId + "/Switches/" + switchId + "/Ports/" + portId,
	}

	return redfishapi.Response(200, redfishapi.VirtualCxlSwitchV100VirtualCxlSwitch{
		//OdataContext: "",
		//OdataEtag:    "",
		OdataId:     "/redfish/v1/Fabrics/" + fabricId + "/Switches/" + switchId + "/VCSs/" + vcsId,
		OdataType:   "#VCSs.V1_0_0.VCSs",
		Actions:     redfishapi.VirtualCxlSwitchV100Actions{},
		Description: "",
		HDMDecoders: 0,
		Id:          vcsId,
		Links:       links,
		...
		Status: redfishapi.ResourceStatus{},
		VCSId:  "",
		VPPBs: redfishapi.VirtualCxlSwitchV100VirtualCxlSwitchVppbs{
			OdataId: fmt.Sprintf("/redfish/v1/Fabrics/%s/Switches/%s/VCSs/%s/VPPBs", fabricId, switchId, vcsId),
		},
	}), nil
}

// RedfishV1FabricsFabricIdSwitchesSwitchIdVCSsVcsIdVPPBsGet -
func (cfm *CfmApiService) RedfishV1FabricsFabricIdSwitchesSwitchIdVCSsVcsIdVPPBsGet(ctx context.Context, fabricId string, switchId string, vcsId string) (redfishapi.ImplResponse, error) {
	appliance, err := manager.GetApplianceById(ctx, fabricId)
	if err != nil {
		return formatRedfishErrorResp(ctx, err.(*common.RequestError))
	}
	...
	for i := 0; i < len(blade.Resources); i++ {
		vppbId := "vppb" + strconv.Itoa(i)
		members = append(members, redfishapi.OdataV4IdRef{
			OdataId: fmt.Sprintf("/redfish/v1/Fabrics/%s/Switches/%s/VCSs/%s/VPPBs/%s", fabricId, switchId, vcsId, vppbId),
		})
	}

	return redfishapi.Response(200, redfishapi.VirtualPci2PciBridgeCollectionVirtualPci2PciBridgeCollection{
		//OdataContext:         "",
		//OdataEtag:            "",
		OdataId:           "/redfish/v1/Fabrics/" + fabricId + "/Switches/" + switchId + "/VCSs/" + vcsId + "/VPPBs",
		OdataType:         "#VPPBs.V1_0_0.VPPBs",
		Description:       "",
		Members:           members,
		MembersodataCount: int64(len(members)),
		//MembersodataNextLink: "",
...
func (cfm *CfmApiService) RedfishV1FabricsFabricIdSwitchesSwitchIdVCSsVcsIdVPPBsVppbIdPatch(ctx context.Context, fabricId string, switchId string, vcsId string, vppbId string, virtualPci2PciBridgeV100VirtualPci2PciBridge redfishapi.VirtualPci2PciBridgeV100VirtualPci2PciBridge) (redfishapi.ImplResponse, error) {
	// TODO : MLD case
	appliance, err := manager.GetApplianceById(ctx, fabricId)
	if err != nil {
		return formatRedfishErrorResp(ctx, err.(*common.RequestError))
	...
	uspId := strings.Replace(vcsId, "vcs", "port", 1)
	r.PortId = uspId
	...
	resourceId, err := blade.GetResourceIdByVppbId(ctx, vppbId)
	if err != nil {
		newErr := fmt.Errorf("error: vppb(%s) cannot find in GetResourceIdByVppbId", vppbId)
		return redfishapi.Response(http.StatusBadRequest, newErr), newErr
	}
	resourceIdInt, _ := strconv.ParseInt(strings.Replace(resourceId, "rb", "", 1), 0, 64)

	var bindingStatus redfishapi.VirtualPci2PciBridgeV100VPpbStatusTypes
	var memoryDetails openapi.MemoryRegion
	var endpoints []redfishapi.OdataV4IdRef
	if virtualPci2PciBridgeV100VirtualPci2PciBridge.BoundLDId == -1 { // Unbind
		r.Operation = "unassign"
		bindingStatus = redfishapi.VIRTUALPCI2PCIBRIDGEV100VPPBSTATUSTYPES_UNBOUND

		// Find memoryId related to vppb
		hasMemory := false
		...
		for _, memoryId := range blade.GetAllMemoryIds(ctx) {
			...
			memoryDetails, _ = memory.GetDetails(ctx)
			if memoryDetails.VppbId == strings.Replace(vppbId, "vppb", "", 1) {
				if strings.Replace(memoryDetails.MemoryAppliancePort, "port", "", 1) == strings.Replace(vcsId, "vcs", "", 1) {
					hasMemory = true
					r.MemoryId = memoryId
		...
		if !hasMemory {
			newErr := fmt.Errorf("error: Memory having VppbId(%s) does not exist", vppbId)
			newRequestErr := &common.RequestError{StatusCode: common.StatusMemoryIdDoesNotExist, Err: newErr} // TODO : status code needs to be changed
			return formatRedfishErrorResp(ctx, newRequestErr)
		}

		_, err = blade.AssignMemory(ctx, &r)
		if err != nil {
			newErr := fmt.Errorf("error: backend assign memory failed")
			return redfishapi.Response(http.StatusBadRequest, newErr), newErr
		}

		_, err := blade.Memory[targetMemoryId].GetDetails(ctx) // to update cache before free memory
		if err != nil {
			newErr := fmt.Errorf("error: backend get memory details failed")
			return redfishapi.Response(http.StatusBadRequest, newErr), newErr
		}

		_, err = blade.FreeMemoryById(ctx, targetMemoryId)
		if err != nil {
			newErr := fmt.Errorf("error: backend free memory failed")
			return redfishapi.Response(http.StatusBadRequest, newErr), newErr
		}
	} else if virtualPci2PciBridgeV100VirtualPci2PciBridge.BoundLDId == 65535 { // Bind
		r.Operation = "assign"
		bindingStatus = redfishapi.VIRTUALPCI2PCIBRIDGEV100VPPBSTATUSTYPES_BOUND_PHYSICAL_PORT
		...
		for _, memoryId := range blade.GetAllMemoryIds(ctx) {
			...
			memoryDetails, _ = memory.GetDetails(ctx)
			// exception : supplied vppb & vcs are already assigned.
			if strings.Replace(memoryDetails.MemoryAppliancePort, "port", "", 1) == strings.Replace(vcsId, "vcs", "", 1) {
				if memoryDetails.VppbId == strings.Replace(vppbId, "vppb", "", 1) {
					newErr := fmt.Errorf("error : supplied vcs(%s) & vppb(%s) is already assigned", vcsId, vppbId)
					return redfishapi.Response(http.StatusBadRequest, newErr), newErr
				}
			}
			// already assigned to other vcs
			if memoryDetails.MemoryAppliancePort != "" {
				continue
			}
		...
		if !hasMemory { // vppb related memory is not composed now, start compose.
			// compose by dsp & ld id & vppb
			var resourceIds []string
			resourceIds = append(resourceIds, resourceId)

			composedMemory, err := blade.ComposeMemoryByResource(ctx, "", resourceIds, vppbId)
			if err != nil {
				newErr := fmt.Errorf("error: Compose Memory Error (Supplied Port(%s) , VppbId(%s)) : %w", resourceId, vppbId, err)
				newRequestErr := &common.RequestError{StatusCode: common.StatusComposePartialSuccess, Err: newErr} // TODO : status code needs to be changed
				return formatRedfishErrorResp(ctx, newRequestErr)
			}
			r.MemoryId = composedMemory.Id
		}

		_, err = blade.AssignMemory(ctx, &r)
		if err != nil {
			return redfishapi.Response(http.StatusBadRequest, fmt.Errorf("error: backend assign memory failed")), nil
		}

		endpoints = append(endpoints, redfishapi.OdataV4IdRef{OdataId: fmt.Sprintf("/redfish/v1/Fabrics/%s/Endpoints/%s", fabricId, switchId+strings.Replace(vcsId, "vcs", "up", 1))})
	} else {
		return redfishapi.Response(http.StatusBadRequest, fmt.Errorf("error: Bound LD ID (%d) is not correctly supplied", virtualPci2PciBridgeV100VirtualPci2PciBridge.BoundLDId)), nil
	}

	return redfishapi.Response(200, redfishapi.VirtualPci2PciBridgeV100VirtualPci2PciBridge{
		//OdataContext:  "",
		//OdataEtag:     "",
		OdataId:       "/redfish/v1/Fabrics/" + fabricId + "/Switches/" + switchId + "/VCSs/" + vcsId + "/VPPBs/" + vppbId,
		OdataType:     "#VPPBs.V1_0_0.VPPBs",
		Actions:       redfishapi.VirtualPci2PciBridgeV100Actions{},
		BindingStatus: bindingStatus,
		BoundLDId:     virtualPci2PciBridgeV100VirtualPci2PciBridge.BoundLDId, // This version only supports SLD (0xFFFF = 65535)
		//BoundPBRId:    -1,
		BoundPortId: resourceIdInt,
		Description: "",
		GCXLID:      nil,
		Id:          vppbId,
		Links: redfishapi.VirtualPci2PciBridgeV100Links{
			Endpoints:           endpoints,
			EndpointsodataCount: int64(len(endpoints)),
			Oem:                 nil,
			Port:                &redfishapi.OdataV4IdRef{OdataId: fmt.Sprintf("/redfish/v1/Fabrics/%s/Switches/%s/Ports/%s", fabricId, switchId, "port"+strings.Replace(resourceId, "rb", "", 1))},
		},
		Name:   "VPPBs",
		Oem:    nil,
		Status: redfishapi.ResourceStatus{},
...
func (cfm *CfmApiService) RedfishV1FabricsFabricIdSwitchesSwitchIdVCSsVcsIdVPPBsVppbIdGet(ctx context.Context, fabricId string, switchId string, vcsId string, vppbId string) (redfishapi.ImplResponse, error) {
	// TODO : MLD case & Multi host share
	...
	if !hasVppbId {
		newErr := fmt.Errorf("error: Invalid vppb(%s)", vppbId)
		newRequestErr := &common.RequestError{StatusCode: common.StatusMemoryIdDoesNotExist, Err: newErr}
		return formatRedfishErrorResp(ctx, newRequestErr)
	}

	resourceId, err := blade.GetResourceIdByVppbId(ctx, vppbId)
	if err != nil {
		newErr := fmt.Errorf("error: vppb(%s) cannot find in backend", vppbId)
		return redfishapi.Response(http.StatusBadRequest, newErr), newErr
	}
	resourceIdInt, _ := strconv.ParseInt(strings.Replace(resourceId, "rb", "", 1), 0, 64)

	for _, memory := range blade.Memory {
		memoryDetails, err := memory.GetDetails(ctx)
		if err != nil {
			return formatRedfishErrorResp(ctx, err.(*common.RequestError))
		}

		if memoryDetails.VppbId == strings.Replace(vppbId, "vppb", "", 1) && memoryDetails.MemoryAppliancePort == strings.Replace(vcsId, "vcs", "", 1) {
			var endpoints []redfishapi.OdataV4IdRef
			var boundLdIdValue int64 = 65535
			bindingStatus := redfishapi.VIRTUALPCI2PCIBRIDGEV100VPPBSTATUSTYPES_BOUND_PHYSICAL_PORT
			endpoints = append(endpoints, redfishapi.OdataV4IdRef{OdataId: fmt.Sprintf("/redfish/v1/Fabrics/%s/Endpoints/%s", fabricId, switchId+strings.Replace(vcsId, "vcs", "up", 1))})
			...
			return redfishapi.Response(200, redfishapi.VirtualPci2PciBridgeV100VirtualPci2PciBridge{
				...
				OdataType:     "#VPPBs.V1_0_0.VPPBs",
				Actions:       redfishapi.VirtualPci2PciBridgeV100Actions{},
				BindingStatus: bindingStatus,  // "BoundLD", "BoundPhysicalPort", "BoundPID", "Busy", "Unbound"
				BoundLDId:     boundLdIdValue, // if only BindingStatus is "BoundLD", This version only supports SLD (0xFFFF = 65535)
				//BoundPBRId:    0,
				BoundPortId: resourceIdInt, // USP ID or DSP ID
				Description: "",
				GCXLID:      nil,
				Id:          vppbId,
				Links: redfishapi.VirtualPci2PciBridgeV100Links{
	...
	return redfishapi.Response(200, redfishapi.VirtualPci2PciBridgeV100VirtualPci2PciBridge{
		...
		OdataType:     "#VPPBs.V1_0_0.VPPBs",
		Actions:       redfishapi.VirtualPci2PciBridgeV100Actions{},
		BindingStatus: redfishapi.VIRTUALPCI2PCIBRIDGEV100VPPBSTATUSTYPES_UNBOUND, // "BoundLD", "BoundPhysicalPort", "BoundPID", "Busy", "Unbound"
		BoundLDId:     -1,
		//BoundPBRId:    0,
		Description: "",
		GCXLID:      nil,
