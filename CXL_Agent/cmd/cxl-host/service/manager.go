...
func GetChassisName() string {
	// Find Chassis name from predefined host-to-switch mapping (hostToSwitchDataStore.json)
	hostName, _ := os.Hostname()
	_, exist := HostToSwitchMap[hostName]
	if !exist {
		panic(fmt.Sprintf("failed to get hostname %s in hostToSwitchMap Unmarshal", hostName))
	}
...
var cxlDevMapWithPPB = map[string]*cxl.CxlDev{}
var cxlDevMapWithXC = map[string]*cxl.CxlDev{}
...
func init() {
	...
	// In case of identifying the upstream port as PCI-TO-PCI Bridge,
	// Another CxlDevList init routine added, not modifying seagate's cxl-util library.
	cxlDevMapWithPPB = cxlutil.InitCxlDevListWithPPB()
	for bdf := range cxlDevMapWithPPB {
		cxlDevOrder = append(cxlDevOrder, bdf)
	}

	// copy map
	for key, value := range cxlDevMapWithPPB {
		cxlDevMap[key] = value
	}

	cxlDevMapWithXC = cxlutil.InitCxlDevListWithXC()
	for bdf := range cxlDevMapWithXC {
		cxlDevOrder = append(cxlDevOrder, bdf)
	}

	for key, value := range cxlDevMapWithXC {
		cxlDevMap[key] = value
	}
...
func GetCXLWithMemDevList() []string {
	...
	for _, bdf := range cxlDevOrder {
		if _, ok := cxlDevMapWithPPB[bdf]; ok {
			continue
		} else if _, ok := cxlDevMapWithXC[bdf]; ok {
			continue
		}
...
var HostToSwitchMap = make(map[string]HostData)
...
func init() {
	...
	for hostName, hostInfo := range hostToSwitchMap["host-data"].(map[string]interface{}) {
		HostToSwitchMap[hostName] = HostData{
			HostIp:          hostInfo.(map[string]interface{})["Host-IP"].(string),
			FabricName:      hostInfo.(map[string]interface{})["Fabric-Name"].(string),
			SwitchName:      hostInfo.(map[string]interface{})["Switch-Name"].(string),
			LinkedUspNumber: hostInfo.(map[string]interface{})["Linked-Usp-Number"].(string),
			CxlSerialNumber: hostInfo.(map[string]interface{})["Cxl-Serial-Number"].(string),
			SwitchIp:        hostInfo.(map[string]interface{})["Switch-IP"].(string),
			SwitchPort:      hostInfo.(map[string]interface{})["Switch-Port"].(string),
		}
	}
}
